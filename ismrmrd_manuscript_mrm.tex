...
\response{We agree completely with the reviewer as to the definition of ``raw data".  We have added a paragraph in the introduction and have hopefully clarified and reinforced the point that raw data means completely untouched by any processing - as close as possible to the output of the analog to digital converters.}
...
\madded[R1.1]{A consistent definition of the term ``raw data" is needed from the outset.  The variations in the definitions of this term within the MRI community are surprising, as evidenced for example by the reactions of various parties during the ISMRM Fat/Water challenge, when ``raw data" was taken to mean complex image data had been coil combined.  Some members of the community held the view this form of pre-processing (coil-combination) was a non-issue, while others held the opposite view and were unhappy that the data had been pre-processed at all.  In the context of this endeavor, the term ``raw data" is defined as MR data that has been completely untouched by any processing whatsoever, i.e. no gridding or ramp sampling correction, coil compression or combination, Fourier transformation, zero padding, etc.  In practical terms, ``raw data" means as close as possible to the output of the instrument's analog to digital converters}
...
Coming soon
\end{center}
\caption{Images reconstructed from an accelerated EPI sequence using SENSE.  Coil sensitivities were estimated from a GRE sequence (not shown).}
\label{fig:epi}
\end{figure}
...
\item Bruker converter: \\
	...
	hash=28db2bcf9eae799f6ec64e556b80c40d34ce319c\\
...
\item Siemens converter:\\
	 ...
	 hash=9d464513f9224cba33c6b674e12443ee3954c172\\
...
The raw data in vendor proprietary formats and in ISMRMRD format are being distributed via \madded[R2.12]{Zenodo} (\url{http://dx.doi.org/10.5281/zenodo.32809}). \mdeleted[R2.12]{GitHub ...XNAT}.  Alternatively the raw data can be downloaded using the \texttt{code/do\_get\_data.sh} script \madded[R2.26]{which is intended for Linux/Apple}. Readers are encouraged to download the data and source code.
